...
| **PLAN.md**(本地) | **活清单**——真机验收欠账(按批次倒排)/ 待拍板常量 / 待做与二期 / 长期观察;只放「还没完的事」 | 新批次落地加一节 watch-items;验过一项删一行 |
| **PLAN-ARCHIVE.md**(本地) | 历次批次的**设计定稿与当日状态**,逐字归档(2026-09-08 前的 PLAN.md 全文;本文件与 notes 里「详见 PLAN §X / PLAN「日期」节」指的就是它) | 新批次的设计叙事追加顶部 |
...
阅读顺序:**本文件(规则)→ docs/notes/(那条规则的来历,按需)→ PLAN.md(还没完的事)→ PLAN-ARCHIVE.md(历次设计与当时状态,按需)→ 代码**。本文件只给「规则 + 在哪兑现 + 一句缘由 + 指针」;**grep 规则出处时连同 `docs/notes/` 与 `docs/AGENT-LOG.md` 一起扫**(few-shot 泄漏源漏扫 `assets/` 的同款教训)。
...
- **新批次的真机验收单 / 待拍板常量 / 待做** → **PLAN.md**(活清单:§1 顶部加一节 watch-items,验过一项删一行;§2 待拍板;§3 待做);**设计定稿与当日状态的叙事** → **PLAN-ARCHIVE.md** 顶部追加(逐字,标日期)。AGENT.md 只引指针。
- **跨会话的个人化观察 / 协作风格 / 踩坑细节** → Claude 记忆(自动维护;它是 point-in-time 观察,**引用前核对现状**,过时就更正)。
- **CLAUDE.md** 只指向本文件,基本不动。

### 现状 / 验收纪律
- **真机验收单(watch-items)在 PLAN.md §1,按批次倒排(2026-09-08 起;之前散在各批次节末,已归档 PLAN-ARCHIVE.md)**:一类东西**只能 Windows 真机 / 真网 / 真钥匙 / 真麦验**(媒体渲染、窗口全屏、代理「直连失败→代理兜底」、和风 JWT 全链、语音功效与唤醒召回、真实视觉应答、并发注入交错气泡…)。**Mac / 浏览器预览跑通的不要当「已验证」声称**,落在 watch-item 里据实标。
...
### 文档分工再申明
- 宪法管**边界**,计划管**还没完的事**(PLAN.md 活清单;设计与历次状态归档 PLAN-ARCHIVE.md),本文件管**规则总纲并指路**。三者不矛盾时以本文件的规则表述为准;若发现 PLAN / 记忆与本文件冲突,**以本文件为规则真相源**,并回头修正不一致的那处。
