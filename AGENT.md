...
- **现状(最新 v0.2.11,2026-07-07 发布)**:MVP 后端 + agent/工具运行时 + 影音 + 任务需知 + 文件能力 + 提醒/web + 语音 + 常驻临场(开机自启/托盘/悬浮窗)+ 远程渠道(TG/钉钉,图/语音全量)+ 多用户(渠道归人 + 声纹归人)+ 主动关怀(悬浮窗关怀候选 + 待办小账)+ 说话人显性化(谁说的进 UI)+ 唤醒确认层(治高频常用名误唤醒)+ AEC 采集(浏览器 getUserMedia 消回声,v0.2.11 转默认)+ 硬件加速转码**均已落地**,核心测试全绿(370+16)。**⚠️ 但 v0.2.7→v0.2.11 这批多数只「Mac 全绿 + 预览验过、Windows 真机待验」**——尤其 v0.2.11 头牌「AEC 采集转默认」改了核心语音链路的默认路径(带一键回退)、钉钉图/语音需真钉钉真网,按 §8.1/§10「Mac 跑通 ≠ 验证通过」这批是当前最大的验收欠账。**权威执行状态 + 真机验收勾项以 PLAN.md 各 § 为准。**
...
### 4.1 命名与调性 🔒
- 项目英文名 **Larkwing**;面向用户中文名 **旺财**(暖萌皮);默认助手名 **BT**(科幻调性;2026-07-10 用户拍板由「7274」改「BT」= BT-7274 的小名,配合「起什么名字就怎么唤醒」——BT 喊得出口〔逼踢〕,「七二七四」保留为默认名下的第二唤醒词,见 §8.2)。
  ...
  - **默认名「BT」三处手工同步**(改默认 = 改这三处):Rust `context::DEFAULT_NAME` · 前端 `pet.name`(`src/locales/*.ts`)· 本条。persona 已是占位符、不再硬编名字,故不在同步清单;默认**唤醒词组**另有单源 `voice/wake.rs::DEFAULT_WAKE_WORDS`(逼踢+七二七四),改默认名连它一起对齐。
...
- 已确立的双源同步点(改其默认即触发本条、逐处对齐):默认名 BT(§4.1;2026-07-10 由 7274 改)· persona 留空(§4.1)· 默认唤醒词组「逼踢+七二七四」(单源 `wake::DEFAULT_WAKE_WORDS`;唤醒词本身已无独立设置 = 名字派生,§8.2)+ 灵敏度 100(§8.2)· 皮肤 scifi(§6.7)· 代理地址(§4.6)。
...
- **图片按 catalog 视觉能力降级(2026-07-04 真机实锤)**:非视觉模型(DeepSeek 全系)收到 `image_url` 直接 API 400(`unknown variant image_url`)→ `catalog::ModelInfo` 加 `vision` 列(数据;未知模型按 false,宁可少看图不可打挂回合),`openai_compat::to_wire` 出向时把图降级成占位文本(「…不支持看图,如实告知」),回合不炸、模型老实说看不了;Gemini/Claude/GPT-5 视觉系原样带图。桌面发图与手机 TG 发图同一条链、同修。
...
- **出站文件 = `send_file` 工具 → `channels/outbound.rs`(2026-07-10 落地;「电脑 ↔ 手机」补齐出站半边,真网/真钉待验)**:模型能把本机文件/图片发到**说话人**的手机(目标恒为 `ToolCtx.user_id` = 渠道归人后的说话人:桌面喊「发我手机」= 主人,家人在手机上要文件 = TA 自己)。目标解析 = 渠道归人映射**反着用**(`resolve_target`:指认给 TA 的线程算 TA 的,TA 是主人时未指认 NULL 的也算;多条取最新绑定;钉钉群聊无 `push_id` 跳过),找不到给明白话(§3.5)。TG = `sendDocument` multipart(≤50MB,一律按文档发保真,reqwest 加 `multipart` feature);钉钉 = **旧接口 `media/upload` 换 mediaId(只认 `gettoken` 旧 token)+ 新 token `batchSend`**(sampleFile;**图片走 sampleImageMsg**——sampleFile 只认办公文档扩展),新旧 token 混用是官方文档姿势、≤20MB。凭证走 secrets、`Target` 的 Debug 只露渠道名(token 绝不进日志)。工具面 = 「tools 依赖 channels 的单向引用」新先例(channels 不认识 tools,不构成环;outbound 不要求 Engine)。
...
- **★唤醒词 = 名字派生,无独立设置(2026-07-10 用户拍板「起什么名字就怎么唤醒」,推翻并取代 2026-06-16「默认唤醒词=小七」条)**:起名叫 A、喊 B 才应、它还自称 A = 糟糕设计——唤醒词不再是独立用户数据,**单源 = 「叫我什么」(`ui.pet_name`)**,`voice/mod.rs::wake_keywords` 每次从名字现派生(`wake::derive_wake_word`):**中文原样、数字转读法(7274→七二七四、小7→小七)、字母缩写按中国人的字母读音(BT→逼踢,`wake::LETTER_READINGS` 26 字母表)**,分隔符/emoji 丢弃;设置页「唤醒词」输入框已删,改名 = 换喊法(前端改完名重启唤醒循环即时生效)。**默认名 BT(没改名)= 默认词组「逼踢 + 七二七四」**(单源 `wake::DEFAULT_WAKE_WORDS`;「小七」退役,升级后喊小七不再应)。**英文单词式名字(≥3 字母且含小写,如 Buddy)派生不出**(按单词念、字母读音救不了,英文 KWS 实测零召回 §7.5)→ 回落默认词组 + `VoiceStatus.wake_fallback` 让 UI 如实提示(§3.5 绝不静默哑掉);单音节名(「B」「一」)能用但预期飘,UI 提示两字以上更好认。**配套关键件:确认层转写归一与派生共用同一张读音表**——`to_syllables` 把 ASCII 按字母/数字中文读法展开(ASR 常吐拉丁原文「BT」,不同表则真呼叫被当幻听拒)。高频常用名(「天天」)误唤醒仍由确认层(下条★)兜。真机待验:逼踢/七二七四真麦召回、字母名标定链。
- **★唤醒确认层 = 命中三段式(2026-07-06 用户拍板落地;精度方向,与上面作废的「召回方向 KWS→ASR 复核」不是一回事——那是为救召回、这是为杀误触)**:唤醒词是用户数据,会被起成**高频常用词**(实锤:小朋友把名字定成「天天」——「天天向上」/「我们天天…」/自播语音电影全线误唤醒)。KWS 保持拉满召回、降级为**候选探测器**;命中后**不立刻出声**:静默续录到断句(`wake.rs` 确认层常量:预滚环 2s / 无续句窗 0.6s / 断句 0.45s / 上限 6s)→ ASR 整句 → **拼音三段式**(`triage_transcript` 纯函数,无调拼音对齐、同音字「甜甜/田田」不误杀;取**末次**出现):
  ...
  - **★即时反馈音「叮」(2026-07-10 用户拍板「旁听保留 + 唤醒不延迟」,取代原「代价:有声应答慢 ~0.5–0.9s」)**:原设计命中后**全程静默**到定夺完才出声 → 用户(尤其不在屏幕前)「很多时候不知道有没有唤醒」。**不删确认层**(旁听/一句话直达/防误应全留),改为把「反馈」与「判定」拆开:命中候选那一刻除点灯+duck 外,**core cpal 直出一声「叮」**(`prompts::play_chirp_async`,**纯正弦短音**——silero VAD 不当人声、ASR 不转字、浏览器采集 AEC3 直接消 → 不污染静默续录/孤立呼名判定;参数单源 `prompts.rs` `CHIRP_*` 可调 §4.11),用户**第一时间知道听到了**;真唤醒/旁听/幻听的判定与「有声应答」仍照旧在后台跑(说话的「嗯?」/ 直达执行仍在定夺后)。**唯一代价**:误命中也会「叮」一下(默认名 BT 不是常用词 → 极少)。
