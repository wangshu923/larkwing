...
- 目标 = Windows(WebView2),开发在 Mac(WKWebView)。**WKWebView 更宽容,会掩盖一整类只在 Windows 暴露的 bug。** 已实锤:
  ...
  - **MSE/shaka 自适应播放只能 Windows/WebView2 真机验**(2026-06-20,§7.1 播放架构换 MSE):拖进度条音画错位是「混流 + ?t= 重启 seek」的固有缺陷,治法 = B 站 DASH 不混流、relay 合成 MPD、前端 shaka 经 MSE 播(播放器管时间轴 → 原生 seek+同步)。Mac 能验的已验(shaka 接入/MSE+avc+aac 支持/原生路径不破/cargo+vue-tsc 全绿),但**真机待验**:① B 站视频经 relay 合成的 MPD 能在 WebView2 里被 shaka 放出来(MSE/CORS/Range 交互、worker);② **拖进度条音画同步且精确**(本病的验收点)、进度条时长正确;③ `probe_sidx` 对真 B 站 .m4s 头能定位到 sidx(探不到会回落混流、seek 仍错 → 看日志「DASH 直供不可用」);④ 放 B 站**不再下 ffmpeg**;⑤ 兼容本地 mp4 仍走 `/f/` 原生(回归不破)。不成立的退路:dash.js,或直连 B 站 playurl 拿 indexRange。
