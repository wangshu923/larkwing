...
- **★唤醒确认层 = 命中三段式(2026-07-06 用户拍板落地;精度方向,与上面作废的「召回方向 KWS→ASR 复核」不是一回事——那是为救召回、这是为杀误触)**:唤醒词是用户数据,会被起成**高频常用词**(实锤:小朋友把名字定成「天天」——「天天向上」/「我们天天…」/自播语音电影全线误唤醒)。KWS 保持拉满召回、降级为**候选探测器**;命中后**不立刻出声**:静默续录到断句(`wake.rs` 确认层常量:预滚环 2s / 无续句窗 0.6s / 断句 0.45s / 上限 6s)→ ASR 整句 → **拼音三段式**(`triage_transcript` 纯函数,无调拼音对齐、同音字「甜甜/田田」不误杀;取**末次**出现):
  ...
  - 铁则:**确认层 fail-open**(ASR 挂了当经典唤醒,绝不因确认层故障叫不应);**规则层绝不无声吞掉可能的真呼叫**(归因不清一律交模型/轻应,只有「转写里根本没有关键词」才静默拒)。代价:有声应答慢 ~0.5–0.9s(视觉秒亮:`WakeCandidate` 事件即点灯+duck)。旁听真输入优先:会话在飞 → 仲裁直接放弃;仲裁中用户打字 → send 的 cancel 把未转正回合整轮挤掉(零痕迹)。eval 守卫 `overheard-dismiss`/`overheard-execute`;集成测试守蒸发/转正/让路。真机 watch-items(误挡率/应答延迟体感/仲裁质量)见 PLAN §11。
