...
- **ASR 模型用户可选(2026-06-20,推翻原「够了不暴露」)**:起因 = 用户实测**小朋友识别率差**。原则上每语言仍有「默认最强组件」(中文 = SenseVoice,快),但**额外放出一个选择器**(设置·声音「识别模型」`voice.asr.model`,app 级,默认 `sense-voice`)给三档:`sense-voice`(快,默认)/ `whisper-small`(Whisper 多语 small,~370MB,**对儿童 / 口音鲁棒性是候选里唯一有实证的**,代价 = autoregressive、CPU 偏慢)/ `firered-ctc`(小红书 FireRedASR2-CTC int8,~740MB,标准普通话最准一档、CTC 快,但**对儿童无实证**——成人 SOTA≠对孩子最好)。仍守「X=数据」:加档 = `voice/models.rs` 一个 `ModelSpec` + `AsrModel` 一支 + `asr.rs` 一个构造分支(架构不同:sense_voice / whisper 双文件 / fire_red_asr_ctc 单文件),`SherpaAsr::transcribe` 与 trait 面不动;缓存带档身份(`mod.rs` 的 `asr: Mutex<Option<(AsrModel, Arc<…>)>>`,换档即重建、同档复用),换档后开着唤醒由前端 `restartWakeIfRunning` 让新模型生效(同 sensitivity)。UI 用友好档名不露模型 ID(收口 §3),hint 诚实标体积/速度+「孩子选听小孩更准」+用时下载+本机处理。**真机 watch-item(只能 Windows 真机 + 真孩子声验)**:① Whisper/FireRed 是否**真改善小朋友**识别(Mac 无法验,且学界提醒「真正的解是拿儿童语料微调,换模型只是次优杠杆」)② Whisper-small CPU 延迟在目标机是否可接受(RTF~1,可能等一两秒;`num_threads=4` 是起点调优值)③ HF resolve 直链国内可达性(hf-mirror 优先;备用 = GitHub release 同名 .tar.bz2,已验 200)。
...
- 目标 = Windows(WebView2),开发在 Mac(WKWebView)。**WKWebView 更宽容,会掩盖一整类只在 Windows 暴露的 bug。** 已实锤:
  ...
  - **MSE/shaka 自适应播放(§7.1 播放架构换 MSE)**:① **B 站 DASH = 2026-06-20 Windows 真机验通 ✓**(拖进度条音画同步,shaka/MSE 在 WebView2 立住);配套 yt-dlp 解析必须 `--proxy ""` 强制直连(否则被用户全局代理带歪、TLS 被掐,§7.1)。② **本地 fMP4-HLS(Stage 2)待真机验**:**先踩过 mpegts 段黑屏**(2026-06-20:shaka 的 mux.js 把 mpegts 视频 transmux 成 fMP4 在 WebView2 失败 append code 3015/3016 → 黑;`-output_ts_offset` 修不了,病在 transmux 不在 PTS)→ 已改 **fMP4 段**(绕开 mux.js,与 B 站 DASH 同路)+ Rust 把段 `tfdt` 改累计起点(`probe::patch_segment_tfdt`,输入 seek 出的分片 tfdt 恒为 0,不修各段堆 0 秒)。Mac 已验(真·ffmpeg 端到端切段→patch→可拼成连续时间轴、单测全绿、shaka 地基已由 B 站验通),Windows 待验 = `/hls/` 的 shaka 把 fMP4 段**拼起来且拖动音画同步**(MSE 拼接是 shaka 行为,Mac 测不了);兼容本地 mp4 仍 `/f/` 原生(回归不破);HEVC 转码段 CPU 是否跟得上。退路:fMP4-HLS 仍不成 → dash.js / 合成本地 DASH MPD / 关键帧对齐切片。
...
*最后整编:2026-06-20(新增 §4.11「写死具体默认值 / 判断须先确认」用户准则;并清理默认唤醒词「小七」的前端硬编码副本)。原合并自 CLAUDE.md(宪法)、PLAN.md(§0–§12 设计与执行状态)及历次会话的踩坑沉淀。*
