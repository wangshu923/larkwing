...
- 目标 = Windows(WebView2),开发在 Mac(WKWebView)。**WKWebView 更宽容,会掩盖一整类只在 Windows 暴露的 bug。** 已实锤:
  ...
  - **MSE/shaka 自适应播放(§7.1 播放架构换 MSE)**:① **B 站 DASH = 2026-06-20 Windows 真机验通 ✓**(拖进度条音画同步,shaka/MSE 在 WebView2 立住);配套 yt-dlp 解析必须 `--proxy ""` 强制直连(否则被用户全局代理带歪、TLS 被掐,§7.1)。② **本地 HLS(Stage 2)待真机验**:Mac 已验(切片有效+段内同步、m3u8 合成、cargo 全绿、shaka 地基已由 B 站验通),Windows 待验 = `/hls/` 的 shaka 把按需 mpegts 段**拼起来且拖动音画同步**(reset-PTS 段跨段拼接是 shaka HLS 行为,Mac 测不了);兼容本地 mp4 仍 `/f/` 原生(回归不破);HEVC 转码段 CPU 是否跟得上。退路:本地 HLS 不成 → dash.js / fMP4 段 / 关键帧对齐切片。
