...
- **AEC 解锁为「采集端浏览器 AEC」路线(2026-07-06 用户拍板,原「明确不做」作废;§9 同步移出)**:自我唤醒的根 = 自播声音(TTS/电影)灌进麦。方案 = 麦克风采集迁前端 `getUserMedia({echoCancellation:true})`——WebView2 = Chromium 自带 **AEC3**,参考信号 = 它自己在播的**全部**音频(relay `/tts/` 与电影 MSE 天然全在内),把桌面软件 AEC 最难的两座山(参考跨 IPC、双时钟对齐)整体绕开;AudioWorklet 重采样 16k 推 core,VAD/KWS/ASR/声纹下游不动(`open_capture` 接缝换源)。**自研/引入软件 AEC 仍不做**。现状 = **采集双源已接(2026-07-06 夜)**:`voice.capture.source`(app 级,cpal 默认零回归 / browser)→ `VoiceRuntime::open_capture_auto` 接缝分派(唤醒/听写/标定/录声纹七个采集点统一走它);browser 源 = 前端 `useMicBridge`(getUserMedia AEC on/NS off/AGC off + AudioWorklet 16k + `voice_push_audio` raw IPC ~10Hz,「core 需要麦」才开不常驻占麦)→ core `push_audio` 扇出 taps(死管自动剪,与「pipe drop 即关麦」同语义)。切换开关暂在试验块(临时 UI,转正后挪正式位);**浏览器源用系统默认麦**(`voice.input_device` 只管 cpal,enumerateDevices 选择器留转正时做)。Mac 矩阵已证:消自播 27~33dB、双讲人声中等音量保留极好、烈度随音量变(PLAN §11 详);**Windows 真机验过才转正默认 + 删 TTS 自激闸门**(`useSpeech.voiceWakeSuspend`,解锁「说话中/放片中喊它打断」两种 barge-in),在那之前 suspend 闸门维持现状。配套 watch-items(藏托盘时采集持续性 / 权限静默放行 / **声纹可能要重录**——采集链换了打分或漂移)记 PLAN §11。**录音媒体避让 duck** 到 20%(唤醒 → 回合收尾区间;确认层落地后**提前到 candidate 即 duck**,给确认 ASR 让路);**Windows「通信活动自动压低」**是 OS 设置(app 改不了,放引导卡)。
