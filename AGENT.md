...
- **不按渠道注入 prompt**(破前缀缓存,§7.5 同理):**出站纯文本**(Telegram 不带 parse_mode 绕开 MarkdownV2 转义坑;钉钉 text 类型)。富格式以后走输出后处理,不走 prompt。
