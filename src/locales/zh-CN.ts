...
export default {
  ...
  tool: {
    ...
    briefing_lookup: '查家庭备忘…',
    briefing_remove: '删一条备忘…',
    skill_lookup: '查技能手册…',
    skill_write: '记一门技能…',
    skill_remove: '删一门技能…',
    reminder_set: '设提醒…',
  ...
  skills: {
    title: '技能',
    tagline: '它会的活儿:出厂带的和你教的;觉得哪条不合适,停用就好',
    back: '返回对话',
    count: '共 {n} 项',
    builtin: '内置',
    taught: '你教的',
    ...
    turnOff: '停用',
    hitsNone: '还没用过',
    hits: '用过 {total} 次 · 近7天 {recent} 次 · 最近 {ago}',
    sections: '附录:{list}',
    confirm: '确定删?',
    empty: '还没有技能。对话里教它一套做法,说「以后就这么办」,就会出现在这里。',
    agoJustNow: '刚刚',
    agoMin: '{n} 分钟前',
    agoHour: '{n} 小时前',
    agoDay: '{n} 天前',
  ...
  settings: {
    ...
    voice: {
      ...
      echoCancel: '回声消除',
      echoCancelHint: '消掉它自己放的声音(电影、音乐、说话声),喊名字更不容易误触;关掉 = 用麦克风原始声音。「自动」= 耳机出声就关(耳机不需要它)、扬声器就开,推荐。',
      aecAuto: '自动',
      aecAutoHp: '现在:耳机在出声 → 已自动关(音质原汁原味)',
      aecAutoSpk: '现在:扬声器在出声 → 已自动开(消掉它自己放的声音)',
      aecAutoUnknown: '现在:正在识别输出设备…',
    },
    remote: {
      enable: '启用',
      status: '状态',
      ...
      weixin: {
        title: '微信',
        hint: '扫码把{name}接进你的微信,在微信里跟 TA 收发消息。走腾讯官方的 iLink 机器人通道,免公网;一人一个机器人,家里人各自扫码、互不顶替。',
        login: '连接',
        scan: '扫码登录',
        relogin: '再绑一个(扫码)',
        ...
        expired: '二维码过期了,点「再绑一个(扫码)」再来一次。',
        blocked: '验证多次没过,先歇一会儿再试。',
        bound: '已绑定',
        legacyBound: '早期绑定(重新扫码可补齐身份)',
        unbind: '解绑',
        unbindFailed: '解绑失败,再试一次。',
        allowed: '额外允许',
        allowedPlaceholder: '绑定过的微信自动放行;一般留空',
        steps: '点「扫码登录」,用手机微信扫码并确认;家里人要用就各自再扫一次,每人一个机器人、同时在线。',
    ...
    system: {
      ...
      dataLocationHint: '所有数据都在这个文件夹。点「备份」导出一份压缩包副本,「恢复」用备份包换回当时的数据,「搬家」可挪到别的盘。',
      backup: '备份…',
      backingUp: '备份中…',
      ...
      restore: '恢复…',
      restoring: '恢复中…',
      restoreConfirm: '用这份备份替换当前数据(聊天、记忆、设置等都会回到备份时的样子;现在的数据会留一份保险副本)。备份内含数据库约 {size} MB、克隆音色 {clones} 段,确认后会自动重启{name}。',
      restoreGo: '确认恢复',
      restoreCancel: '取消',
      restoreFailed: '恢复没成功,数据仍是原样。',
      restoreErr: {
        not_backup: '这不像一份备份包(里面没有数据库)。',
      ...
      autoBackupLast: '上次备份:{time} · 每 {days} 天一份,保留最近 {keep} 份',
      autoBackupPending: '还没备过,到点会自动备第一份。',
      autoBackupErr: '上次没备成:{err}',
      autoBackupFirstFailed: '第一份备份没成功,看看那个文件夹能不能写入。',
      autoBackupHint: '开启后定期自动备一份到你选的文件夹,只留最近几份、更旧的自动清掉(改过名的备份不会动)。备到另一块盘或移动硬盘最保险。',
      oldData: '旧数据',
      oldDataDelete: '删除旧数据',
      oldDataKeep: '保留',
      dataErr: {
      ...
      credTitle: '下载认证',
      credHint: '给需要账号的网址(自家 NAS、坚果云这类 WebDAV、或把网盘挂成 WebDAV 的服务)存一份账号,下载时按网址自动带上。密码存进系统密钥串、只进不出——所以这里只显示网址;要改密码就用同一个网址再存一次。',
      credEmpty: '还没有配过',
      credAdd: '添加',
      credSave: '保存',
      credRemove: '删除',
      credHostPlaceholder: '网址,例如 dav.jianguoyun.com',
      credUser: '账号',
      credPassword: '密码',
      credCancel: '取消',
      credSaveFailed: '没保存成,再试一次',
      about: '关于',
      version: '版本',
