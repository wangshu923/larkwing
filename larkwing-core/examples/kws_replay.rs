...
fn main() {
    ...
    let configs: [(&str, bool, f32, f32); 9] = [
        ("s1.5 t0.10  ← 真·当前默认(灵敏100)", true, 1.5, 0.10),
        ("s1.5 t0.15  ← 灵敏~85           ", true, 1.5, 0.15),
        ("s1.5 t0.20  ← 灵敏50/robot 阈值  ", true, 1.5, 0.20),
        ("s1.5 t0.30  ← 仅降阈到 0.30     ", true, 1.5, 0.30),
        ("s1.5 t0.45  ← 旧默认(已废)      ", true, 1.5, 0.45),
