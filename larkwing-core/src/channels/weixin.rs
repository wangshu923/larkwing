...
pub(super) async fn run(ctx: Arc<ChannelCtx>, ct: CancellationToken) {
    ...
    while !ct.is_cancelled() {
        ...
        match serve(&ctx, &net, &ct).await {
            ...
            Err(e) => {
                let msg = format!("{e:#}");
                tracing::warn!(err = %msg, "微信渠道出错,5s 后重连");
...
async fn serve(ctx: &ChannelCtx, net: &net::Client, ct: &CancellationToken) -> Result<()> {
    let token = ctx.secret("remote.weixin.token").context("没配微信 token(先扫码登录)")?;
    let base = base_url_of(ctx);
    let allowed = allowed_users(ctx);
    let mut buf = ctx
        .engine
        .store()
        .settings
        .get(None, UPDATES_BUF_KEY)
        .ok()
        .flatten()
        .unwrap_or_default();
    tracing::info!(allow = allowed.len(), has_cursor = !buf.is_empty(), "微信渠道在线");
    ...
    loop {
        ...
        if let Some(nb) = resp.get("get_updates_buf").and_then(Value::as_str) {
            if !nb.is_empty() && nb != buf {
                buf = nb.to_string();
                let _ = ctx.engine.store().settings.set(None, UPDATES_BUF_KEY, &buf);
            }
        }
...
pub(super) fn base_url_of(ctx: &ChannelCtx) -> String {
    ctx.setting("remote.weixin.base_url").unwrap_or_else(|| DEFAULT_BASE_URL.to_string())
}
...
fn store_login(engine: &crate::engine::Engine, token: &str, base_url: &str, user_id: &str) -> Result<()> {
    ...
    crate::secrets::set(settings, "remote.weixin.token", token).context("保存微信 token 失败")?;
    if base_url != DEFAULT_BASE_URL {
        settings.set(None, "remote.weixin.base_url", base_url)?;
    } else {
        let _ = settings.delete(None, "remote.weixin.base_url");
    }
    // 扫码人自动进白名单(空名单 = onboarding 模式;加了扫码人 = 只放行 TA,与 TG 报 chat id 同哲学)
    if !user_id.is_empty() {
        let cur = settings.get(None, "remote.weixin.allowed_users").ok().flatten().unwrap_or_default();
        if !cur.split([',', '，', ' ', ';', '；', '\n']).any(|x| x.trim() == user_id) {
            let next = if cur.trim().is_empty() { user_id.to_string() } else { format!("{cur},{user_id}") };
            settings.set(None, "remote.weixin.allowed_users", &next)?;
        }
    }
    // 换号/重登:清旧 getupdates 游标,避免用别的账号的游标拉空/错乱
    let _ = settings.delete(None, UPDATES_BUF_KEY);
    Ok(())
}
