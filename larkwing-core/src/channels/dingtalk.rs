...
fn parse_bot_message(frame: &Value) -> Option<BotMessage> {
    ...
    let ext_id = if data.get("conversationType").and_then(Value::as_str) == Some("2") {
        // 群聊:按发言人隔离(robot 坑#2),否则同群多人互串会话
        let sender = data.get("senderStaffId").and_then(Value::as_str).unwrap_or("");
        format!("{conv_id}:{sender}")
    } else {
        conv_id.to_string()
    };
    let sender = data
