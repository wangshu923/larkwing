...
pub(super) enum WakeCmd {
    ...
    /// 回合失败/取消/不念:直接回待唤醒。
    Resume,
    /// 口头确认(§7.8 确认闸):回合在飞撞高危动作、前端判定这张卡属于当前语音回合 →
    /// 开一段听音(「确认/可以」= 允许、「不要/算了」= 拒、听不清不算数回落等卡)。
    /// 只在 AwaitTurn 相位生效(回合正飞着;别的相位没有「问句刚念完」的语境)。
    ConfirmListen { id: u64 },
}
...
/// 口头确认的开口窗(§7.8):TTS 问句与听音并行(browser 源 AEC 消自播),问句 ~3-4s,
/// 给 10s 等人开口;开口后断句/上限沿听写链 hangover / MAX_SPEECH_S。只听一轮,
/// 听不清不算数(「宁可不认」)——卡片还在屏幕上等满总超时。
const CONFIRM_LISTEN_WINDOW: Duration = Duration::from_secs(10);
...
enum Phase {
    ...
    /// 口头确认听音(§7.8):听一轮应答 → 词表判定 → resolve;完事回 AwaitTurn(回合还在飞)。
    ConfirmListen { id: u64 },
}
...
impl Phase {
    fn label(&self) -> &'static str {
        match self {
            ...
            Phase::ConfirmListen { .. } => "ConfirmListen",
        }
    }
}
    ...
    loop {
        ...
        loop {
            match cmd.try_recv() {
                ...
                Ok(WakeCmd::ConfirmListen { id }) => {
                    // 只在回合在飞时有意义(确认发生在工具执行中);别的相位丢弃(卡片路兜)
                    if matches!(phase, Phase::AwaitTurn(_)) {
                        phase = Phase::ConfirmListen { id };
                    }
                }
                Ok(WakeCmd::Resume) => {
                    pipe.drain();
        ...
        match phase {
            ...
            Phase::ConfirmListen { id } => {
                // 口头确认(§7.8):听一轮应答 → 严格词表(confirm::voice_reply,代码判定)
                // → resolve;听不清/含糊**不算数**(「宁可不认」——误拒代价是再说一遍,误允
                // 的代价是钱),卡片继续等满总超时。完事回 AwaitTurn(回合还在飞)。
                let confirmer = d.rt.confirmer();
                let still_pending =
                    confirmer.as_ref().map(|c| c.has_pending(id)).unwrap_or(false);
                if !still_pending {
                    // 念问句期间别处已点头/摇头:不用听了
                    phase = Phase::AwaitTurn(Instant::now());
                    continue;
                }
                pipe.drain();
                vad.reset();
                ...
                match transcribe(d, out)? {
                    ...
                    None => {
                        tracing::info!("口头确认:窗内没人开口,交回卡片等");
                    }
                }
                d.rt.publish(VoiceEvent::State { phase: VoicePhase::Idle });
                phase = Phase::AwaitTurn(Instant::now());
