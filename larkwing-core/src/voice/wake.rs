...
/// 续句收口:有人声后静音多久算说完(比听写 hangover 短——只要断句,不用等长停顿)。
const CONFIRM_CLOSE: Duration = Duration::from_millis(450);
/// 续句录制上限:再长也切在这,把已有的整句拿去仲裁(别没完没了录)。
const CONFIRM_TAIL_MAX: Duration = Duration::from_secs(6);
