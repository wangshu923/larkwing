...
## 聊完了就收尾
〔语音〕回合里,你和用户之间本来是免唤醒连着聊的——一句答完,系统会短暂留着听、TA 不用再喊你就能接着说。如果你判断这轮交流已经结束、TA 多半不会再接话(明确说了再见/没事了/先这样,或者事情已经办完、话到此为止),就在回复的同时调用 end_conversation:调了它,你正常把这句话说完(该道别就自然道个别),之后系统会安静回到待唤醒,不再免唤醒接话。**默认是继续留着听**——只要你还拿不准 TA 想不想接着说,就别调 end_conversation,让对话自然接下去(宁可多留一会儿,也别把还想说话的人赶走)。没有〔语音〕的回合(打字)不用管这个。
...
## 旁听(名字被喊到)
用户消息开头带〔旁听〕= 你的名字刚在屋里被提到,但那句话**不一定是对你说的**:可能是真在叫你办事,也可能是电视/节目里带到、家里人聊天顺嘴提起。整句话都在消息里,自己判断:确实在叫你(给你指令、问你话)→ 正常回应或直接办;明显不是(节目名、别人对话里的词、跟你无关的句子)→ 只回 __IGNORE__ 这一个词,什么都不说;真拿不准 → 轻轻应一声问是不是在叫你(宁可多应一声,别把真在叫你的人晾着)。〔旁听〕是系统加的输入形态标记,不是用户打的字,绝不复述它。
...
## 出厂示范
对话最开头以【示范】开头的几段是出厂教学样例,**只用来学怎么用工具、怎么说话**。里面的人物、称呼、地名、路径、文件、片名/歌名,以及工具查到的结果(本地有没有某文件、搜到什么、想起什么)**全部是编的**,与眼前这位用户无关:既不能当作用户的真实信息去引用或称呼,也绝不能当成你自己已经知道的事实——该查证的仍要现查现问,不拿示范里的结论当答案;**往记忆、家庭备忘等真实数据里写内容时,只能写眼前用户真实说过的信息,示范里的路径/地点/名字一个字也不能混进去**。真实对话从示范之后开始。";
...
/// (event_injection 同族:确定性包装、人格中立)。行为要求全在消息里、刻意不进 LAWS ——
/// 子回合与聊天回合共享同一份稳定前缀模板,靠消息内容分化(§4.8)。
pub(super) fn delegate_injection(task: &str) -> String {
    // 「你手上只有直接干活的工具」一句 = 评审实锤的坑型修复(承诺与工具面脱节,send_file
    // 渠道清单同族):子回合共享聊天前缀,LAWS「干长活」点名的 plan_set/delegate 与
    // task_status/task_cancel 在子回合工具面里都被排除 —— 不当场说清,模型会照法条调
    // 不存在的工具白烧一轮。前缀模板刻意不分叉(§4.8),免责就落在这条注入消息里。
    format!(
        "【分派的活】主回合把下面这件事派给你独立完成。你看不到派活之前的对话,\
         这份简报就是全部上下文,别猜也别反问。你手上只有直接干活的工具:不用列计划清单、\
         也没有再往下分派的通道,步骤自己一步步干完;转了后台的活(下载/加工)你查不了\
         进度也停不了——把编号写进汇报,主回合会接手盯。办完把**结果要点**直接写出来当汇报\
         (关键结论、路径、数字、清单),别复述过程;大段产出(调研正文、长清单)\
         先用 fs_write_text 写成文件放在用户找得到的地方,汇报里给路径和摘要;\
         办不成就如实说卡在哪,绝不编。任务:{task}"
    )
}
    ...
    for msg in history {
        match msg.role.as_str() {
            ...
            "user" => {
                ...
                // 旁听标记在最前(仲裁语义罩住整句;「旁听」法条按它生效)
                if meta.input == "overheard" {
                    prefix.push_str("〔旁听〕");
                }
                if meta.speak {
                    prefix.push_str("〔语音〕");
...
mod tests {
    ...
    fn laws_and_briefings_join_system_in_stable_order() {
        ...
        // 「干长活」法条(§6.5 计划槽):点名 plan_set(BASE_TOOLS 存在理由 = 法条点名),
        // 且把「没干完别停下来问」的行为纪律立在常驻层(工具描述是拉取式的,治不了这半)。
        assert!(req.system.contains("## 干长活"), "干长活法条必须进 LAWS");
        assert!(req.system.contains("plan_set"), "法条点名 plan_set");
        assert!(req.system.contains("别中途停下来问"), "「没干完别停」的纪律在常驻层");
